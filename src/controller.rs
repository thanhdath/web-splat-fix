...
impl CameraController {
    ...
    pub fn process_keyboard(&mut self, key: KeyCode, pressed: bool) -> bool {
        let amount = if pressed { 20.0 } else { 0.0 };
        ...
        let processed = match key {
            ...
            KeyCode::KeyQ => {
                self.rotation.x += amount; // / self.sensitivity;
                true
            }
            KeyCode::KeyE => {
                self.rotation.x += -amount;// / self.sensitivity;
    ...
    pub fn process_mouse(&mut self, x: f32, y: f32) {
        if self.left_mouse_pressed {
            ...
            if self.last_rotation_x == 0.0 {
                self.last_rotation_x = x;
            } 

            if self.last_rotation_y == 0.0 {
                self.last_rotation_y = y;
            }
            let dx = (x - self.last_rotation_x) / 50.0;
            let dy = (y - self.last_rotation_y) / 50.0;

            self.rotation.x = self.rotation.x + dx as f32;
            self.rotation.y = self.rotation.y + dy as f32;
        ...
        if self.right_mouse_pressed {
            if self.last_shift_x == 0.0 {
                self.last_shift_x = x;
            }
            if self.last_shift_y == 0.0 {
                self.last_shift_y = y;
            }
            let dx = (x - self.last_shift_x) / 50.0;
            let dy = (y - self.last_shift_y) / 50.0;
            println!("Shift x = {}, y = {}", self.shift.x, self.shift.y);
            println!("dx = {}, dy = {}", dx, dy);
            self.shift.y -= dx;
            self.shift.x += dy;
            self.last_shift_x = x;
